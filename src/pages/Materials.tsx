...
function MaterialModal({ material, suppliers, onClose }: { material: Material | null, suppliers: Supplier[], onClose: () => void }) {
  ...
  const [formData, setFormData] = useState<Partial<Material>>({
    ...
    costPerSqm: material?.costPerSqm || 0,
    sellPerSqm: material?.sellPerSqm || 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        costPerSqm: Number(formData.costPerSqm) || 0,
        sellPerSqm: Number(formData.sellPerSqm) || 0,
      ...
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200 relative mb-10 sm:mb-20">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-10 space-y-8">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                <label className="block text-[10px] font-black text-text-light uppercase tracking-[0.3em] mb-3">Cost Price (m²)</label>
                <div className="relative group">
                  <div className="absolute left-6 top-1/2 -translate-y-1/2 text-text-light font-black group-focus-within:text-brand-accent transition-colors">R</div>
                  <input 
                    type="number"
                    step="0.01"
                    value={isNaN(formData.costPerSqm as number) ? '' : formData.costPerSqm}
                    onChange={(e) => setFormData({ ...formData, costPerSqm: parseFloat(e.target.value) })}
                    className="w-full pl-12 pr-6 py-4 bg-gray-50 border border-border rounded-2xl font-bold tabular-nums focus:outline-none focus:ring-4 focus:ring-brand-accent/5 focus:border-brand-accent"
                    placeholder="0.00"
                  />
                </div>
              ...
              <div>
                <label className="block text-[10px] font-black text-text-light uppercase tracking-[0.3em] mb-3">Sell Price (m²)</label>
                <div className="relative group">
                  <div className="absolute left-6 top-1/2 -translate-y-1/2 text-text-light font-black group-focus-within:text-brand-accent transition-colors">R</div>
                  <input 
                    type="number"
                    step="0.01"
                    value={isNaN(formData.sellPerSqm as number) ? '' : formData.sellPerSqm}
                    onChange={(e) => setFormData({ ...formData, sellPerSqm: parseFloat(e.target.value) })}
                    className="w-full pl-12 pr-6 py-4 bg-gray-50 border border-border rounded-2xl font-bold tabular-nums focus:outline-none focus:ring-4 focus:ring-brand-accent/5 focus:border-brand-accent"
                    placeholder="0.00"
                  />
                </div>
      ...
      <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl flex flex-col md:flex-row overflow-hidden animate-in fade-in zoom-in duration-200 relative mb-10 sm:mb-20">
        ...
        <div className="w-full md:w-[280px] bg-paper p-10 flex flex-col border-r border-border shrink-0 md:overflow-y-auto">
          ...
          <div className="mt-auto space-y-6">
            ...
            <div>
              <span className="text-[8px] font-black text-text-light uppercase tracking-widest block mb-1 opacity-50">Costing Metric (Base)</span>
              <p className="text-xl font-black text-brand-accent italic tracking-tighter">R{material.costPrice.toFixed(2)} / {material.unit}</p>
            </div>
            {material.costPerSqm != null && material.sellPerSqm != null && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-[8px] font-black text-text-light uppercase tracking-widest block mb-1 opacity-50">Cost (m²)</span>
                  <p className="text-lg font-black text-text-main italic tracking-tighter">R{material.costPerSqm.toFixed(2)}</p>
                </div>
                <div>
                  <span className="text-[8px] font-black text-text-light uppercase tracking-widest block mb-1 opacity-50">Sell (m²)</span>
                  <p className="text-lg font-black text-text-main italic tracking-tighter">R{material.sellPerSqm.toFixed(2)}</p>
                </div>
              </div>
            )}
          </div>
